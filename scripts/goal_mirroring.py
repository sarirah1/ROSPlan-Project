...
# Add or remove the goal to the knowledge base (kb)
...
# Generate the problem and plan using the planner from the kb
...
# Execute the plan using simulated actions
def execute_plan():
    # rospy.sleep(25.0)
...
# Check if the plan is solved for Metric-FF planner
...
# Update the numeric facts in kb since simulated actions does not do this
...
def update_food_amount(robot_param, split_actions, robot_1_food_amount, robot_2_food_amount, robot_3_food_amount, robot_4_food_amount):
    # Remove food amount and replace it by decrease it
    kus = KnowledgeUpdateServiceRequest()
    # Remove knowledge flag
    kus.knowledge.knowledge_type = 2
    kus.update_type = 2
    kus.knowledge.attribute_name = "food-amount"
    kv = KeyValue()
    kv.value = split_actions[robot_param]
    kv.key = get_object_type(kv.value)
    kus.knowledge.values.append(kv)
    kuc = rospy.ServiceProxy('/rosplan_knowledge_base/update', KnowledgeUpdateService)
    kb = kuc(kus)
    ...
    if not kb:
        rospy.logerr("KCL: (%s) Food amount was not removed!" % rospy.get_name())

    kus = KnowledgeUpdateServiceRequest()
    kus.update_type = 0
    kus.knowledge.knowledge_type = 2
    kus.knowledge.attribute_name = "food-amount"
    if "robot_1" == split_actions[robot_param]:
        kus.knowledge.function_value = robot_1_food_amount - 1
    if "robot_2" == split_actions[robot_param]:
        kus.knowledge.function_value = robot_2_food_amount - 1
    if "robot_3" == split_actions[robot_param]:
        kus.knowledge.function_value = robot_3_food_amount - 1
    if "robot_4" == split_actions[robot_param]:
        kus.knowledge.function_value = robot_4_food_amount - 1
    ...
    if not kb:
        rospy.logerr("KCL: (%s) Food amount was not added!" % rospy.get_name())

def update_water_amount(robot_param, split_actions, robot_1_water_amount, robot_2_water_amount, robot_3_water_amount, robot_4_water_amount):
    # Remove food amount and replace it by decrease it
    kus = KnowledgeUpdateServiceRequest()
    # Remove knowledge flag
    kus.update_type = 2
    kus.knowledge.knowledge_type = 2
    kus.knowledge.attribute_name = "water-amount"
    kv = KeyValue()
    kv.value = split_actions[robot_param]
    kv.key = get_object_type(kv.value)
    kus.knowledge.values.append(kv)
    ...
    if not kb:
        rospy.logerr("KCL: (%s) Water amount was not removed!" % rospy.get_name())
    ...
    kus.knowledge.attribute_name = "water-amount"
    if "robot_1" == split_actions[robot_param]:
        kus.knowledge.function_value = robot_1_water_amount - 1
    if "robot_2" == split_actions[robot_param]:
        kus.knowledge.function_value = robot_2_water_amount - 1
    if "robot_3" == split_actions[robot_param]:
        kus.knowledge.function_value = robot_3_water_amount - 1
    if "robot_4" == split_actions[robot_param]:
        kus.knowledge.function_value = robot_4_water_amount - 1
    ...
    if not kb:
        rospy.logerr("KCL: (%s) Water amount was not added!" % rospy.get_name())


def update_battery_amount(robot_param, split_actions, robot_1_battery, robot_2_battery, robot_3_battery, robot_4_battery, robot_1_velocity, robot_2_velocity, robot_3_velocity, robot_4_velocity, battery_discharge, value):
    ...
    kus.knowledge.attribute_name = "battery-amount"
    if "robot_1" == split_actions[robot_param]:
        kus.knowledge.function_value = robot_1_battery - ((value/robot_1_velocity) * battery_discharge)
    if "robot_2" == split_actions[robot_param]:
        kus.knowledge.function_value = robot_2_battery - ((value/robot_2_velocity) * battery_discharge)
    if "robot_3" == split_actions[robot_param]:
        kus.knowledge.function_value = robot_3_battery - ((value/robot_3_velocity) * battery_discharge)
    if "robot_4" == split_actions[robot_param]:
        kus.knowledge.function_value = robot_4_battery - ((value/robot_4_velocity) * battery_discharge)
...
def recharge_battery(robot_param, split_actions, battery_capacity):
    # Remove battery amount and replace it by decrease it according to velocity
    kus = KnowledgeUpdateServiceRequest()
    # Remove knowledge flag
    kus.update_type = 2
...
def recharge_food_amount(robot_param, split_actions, food_capacity):
    # Remove battery amount and replace it by decrease it according to velocity
    kus = KnowledgeUpdateServiceRequest()
    # Remove knowledge flag
    kus.update_type = 2
    ...
    kus.knowledge.knowledge_type = 2
    kus.knowledge.attribute_name = "food-amount"
    kus.knowledge.function_value = food_capacity
...
def recharge_water_amount(robot_params, split_actions, water_capacity):
    # Remove battery amount and replace it by decrease it according to velocity
    kus = KnowledgeUpdateServiceRequest()
    # Remove knowledge flag
    kus.update_type = 2
    ...
    kus.knowledge.attribute_name = "water-amount"
    kv = KeyValue()
    kv.value = split_actions[robot_params]
    kv.key = get_object_type(kv.value)
    kus.knowledge.values.append(kv)
    kuc = rospy.ServiceProxy('/rosplan_knowledge_base/update', KnowledgeUpdateService)
    kb = kuc(kus)
    ...
    kus.knowledge.knowledge_type = 2
    kus.knowledge.attribute_name = "water-amount"
    kus.knowledge.function_value = water_capacity
...
def update_input_amount(robot_params, split_actions, robot_1_input_amount, robot_2_input_amount, robot_3_input_amount, robot_4_input_amount):
    # Remove food amount and replace it by decrease it
    kus = KnowledgeUpdateServiceRequest()
    # Remove knowledge flag
    kus.update_type = 2
    ...
    kus.knowledge.knowledge_type = 2
    kus.knowledge.attribute_name = "input-amount"
    if "robot_1" == split_actions[robot_params]:
        kus.knowledge.function_value = robot_1_input_amount - 1
    if "robot_2" == split_actions[robot_params]:
        kus.knowledge.function_value = robot_2_input_amount - 1
    if "robot_3" == split_actions[robot_params]:
        kus.knowledge.function_value = robot_3_input_amount - 1
    if "robot_4" == split_actions[robot_params]:
        kus.knowledge.function_value = robot_4_input_amount - 1
...
def recharge_input(robot_params, split_actions, input_capacity):
    # Remove battery amount and replace it by decrease it according to velocity
    kus = KnowledgeUpdateServiceRequest()
    # Remove knowledge flag
    kus.update_type = 2
...
def update_numeric_values_kb():
    # robot_1_battery, robot_2_battery, robot_3_battery, robot_4_battery, robot_1_input_amount, robot_2_input_amount, robot_3_input_amount, robot_4_input_amount, battery_discharge, battery_capacity, input_capacity, water_capacity, food_capacity, robot_1_velocity, robot_2_velocity, robot_3_velocity, robot_4_velocity, robot_1_food_amount, robot_2_food_amount, robot_3_food_amount, robot_4_food_amount, robot_1_water_amount, robot_2_water_amount, robot_3_water_amount, robot_4_water_amount, mission_length = retrieve_function_values_in_problem_file()
    ...
    for l in obs_lines:
        ...
        get_functions_service = rospy.ServiceProxy('/rosplan_knowledge_base/state/functions', GetAttributeService)
        functions = get_functions_service().attributes
        robot_1_food_amount = -1
        robot_2_food_amount = -1
        robot_3_food_amount = -1
        robot_4_food_amount = -1

        robot_1_battery_amount = -1
        robot_2_battery_amount = -1
        robot_3_battery_amount = -1
        robot_4_battery_amount = -1

        robot_1_water_amount = -1
        robot_2_water_amount = -1
        robot_3_water_amount = -1
        robot_4_water_amount = -1

        robot_1_input_amount = -1
        robot_2_input_amount = -1
        robot_3_input_amount = -1
        robot_4_input_amount = -1
        ...
        for func in functions:
            ...
            elif func.attribute_name == "food-amount" and func.values[0].value == "robot_1":
                robot_1_food_amount = func.function_value
            elif func.attribute_name == "food-amount" and func.values[0].value == "robot_2":
                robot_2_food_amount = func.function_value
            elif func.attribute_name == "food-amount" and func.values[0].value == "robot_3":
                robot_3_food_amount = func.function_value
            elif func.attribute_name == "food-amount" and func.values[0].value == "robot_4":
                robot_4_food_amount = func.function_value
            
            elif func.attribute_name == "battery-amount" and func.values[0].value == "robot_1":
                robot_1_battery_amount = func.function_value
            elif func.attribute_name == "battery-amount" and func.values[0].value == "robot_2":
                robot_2_battery_amount = func.function_value
            elif func.attribute_name == "battery-amount" and func.values[0].value == "robot_3":
                robot_3_battery_amount = func.function_value
            elif func.attribute_name == "battery-amount" and func.values[0].value == "robot_4":
                robot_4_battery_amount = func.function_value 

            elif func.attribute_name == "velocity" and func.values[0].value == "robot_1":
                robot_1_velocity = func.function_value
            elif func.attribute_name == "velocity" and func.values[0].value == "robot_2":
                robot_2_velocity = func.function_value
            elif func.attribute_name == "velocity" and func.values[0].value == "robot_3":
                robot_3_velocity = func.function_value
            elif func.attribute_name == "velocity" and func.values[0].value == "robot_4":
                robot_4_velocity = func.function_value 
            elif func.attribute_name == "discharge-rate-battery":
                battery_discharge = func.function_value
            
            elif func.attribute_name == "water-amount" and func.values[0].value == "robot_1":
                robot_1_water_amount = func.function_value
            elif func.attribute_name == "water-amount" and func.values[0].value == "robot_2":
                robot_2_water_amount = func.function_value
            elif func.attribute_name == "water-amount" and func.values[0].value == "robot_3":
                robot_3_water_amount = func.function_value
            elif func.attribute_name == "water-amount" and func.values[0].value == "robot_4":
                robot_4_water_amount = func.function_value 

            elif func.attribute_name == "battery-capacity":
                battery_capacity = func.function_value 
            elif func.attribute_name == "food-capacity":
                food_capacity = func.function_value 
            elif func.attribute_name == "water-capacity":
                water_capacity = func.function_value 
            elif func.attribute_name == "input-capacity":
                input_capacity = func.function_value

            elif func.attribute_name == "input-amount" and func.values[0].value == "robot_1":
                robot_1_input_amount = func.function_value
            elif func.attribute_name == "input-amount" and func.values[0].value == "robot_2":
                robot_2_input_amount = func.function_value
            elif func.attribute_name == "input-amount" and func.values[0].value == "robot_3":
                robot_3_input_amount = func.function_value
            elif func.attribute_name == "input-amount" and func.values[0].value == "robot_4":
                robot_4_input_amount = func.function_value 

        if split_actions[0] == "feed_livestock":
            update_mission_length(mission_length, amount=250)
            update_food_amount(3, split_actions, robot_1_food_amount, robot_2_food_amount, robot_3_food_amount, robot_4_food_amount)
            update_battery_amount(3, split_actions, robot_1_battery_amount, robot_2_battery_amount, robot_3_battery_amount, robot_4_battery_amount, robot_1_velocity, robot_2_velocity, robot_3_velocity, robot_4_velocity, battery_discharge, value=250)
            

        elif split_actions[0] == "gather_produce":
            update_mission_length(mission_length, amount=20)            
            update_battery_amount(2, split_actions, robot_1_battery_amount, robot_2_battery_amount, robot_3_battery_amount, robot_4_battery_amount, robot_1_velocity, robot_2_velocity, robot_3_velocity, robot_4_velocity, battery_discharge, value=100)

        elif split_actions[0] == "water_crop":
            update_mission_length(mission_length, amount=100)
            update_water_amount(2, split_actions, robot_1_water_amount, robot_2_water_amount, robot_3_water_amount, robot_4_water_amount)
            update_battery_amount(2, split_actions, robot_1_battery_amount, robot_2_battery_amount, robot_3_battery_amount, robot_4_battery_amount, robot_1_velocity, robot_2_velocity, robot_3_velocity, robot_4_velocity, battery_discharge, value=500)

        elif split_actions[0] == "recharge_battery_ground":
            update_battery_amount(1, split_actions, robot_1_battery_amount, robot_2_battery_amount, robot_3_battery_amount, robot_4_battery_amount, robot_1_velocity, robot_2_velocity, robot_3_velocity, robot_4_velocity, battery_discharge, value=500)
            recharge_battery(1, split_actions, battery_capacity)

        elif split_actions[0] == "drop_crop_in_box":
            update_mission_length(mission_length, amount=20)
            update_battery_amount(2, split_actions, robot_1_battery_amount, robot_2_battery_amount, robot_3_battery_amount, robot_4_battery_amount, robot_1_velocity, robot_2_velocity, robot_3_velocity, robot_4_velocity, battery_discharge, value=100)

        elif split_actions[0] == "drop_produce_in_box":
            update_mission_length(mission_length, amount=20)
            update_battery_amount(2, split_actions, robot_1_battery_amount, robot_2_battery_amount, robot_3_battery_amount, robot_4_battery_amount, robot_1_velocity, robot_2_velocity, robot_3_velocity, robot_4_velocity, battery_discharge, value=100)

        elif split_actions[0] == "recharge_food_amount":
            recharge_food_amount(2, split_actions, food_capacity)

        elif split_actions[0] == "recharge_water_amount":
            recharge_water_amount(2, split_actions, water_capacity)

        elif split_actions[0] == "move_robot":
            start = split_actions[1]
            end = split_actions[2]
            
            for func in functions:
                if func.attribute_name == "distance_location" and func.values[0].value == start and func.values[1].value == end:
                    dist = func.function_value
            update_battery_amount(3, split_actions, robot_1_battery_amount, robot_2_battery_amount, robot_3_battery_amount, robot_4_battery_amount, robot_1_velocity, robot_2_velocity, robot_3_velocity, robot_4_velocity, battery_discharge, value=dist)
            update_mission_length(mission_length, amount=dist)

        elif split_actions[0] == "go_to":
            start = split_actions[1]
            end = split_actions[2]
            ...
            update_battery_amount(2, split_actions, robot_1_battery_amount, robot_2_battery_amount, robot_3_battery_amount, robot_4_battery_amount, robot_1_velocity, robot_2_velocity, robot_3_velocity, robot_4_velocity, battery_discharge, value=1000)
            update_mission_length(mission_length, amount=1000)

        elif split_actions[0] == "pulverize_region":
            update_battery_amount(2, split_actions, robot_1_battery_amount, robot_2_battery_amount, robot_3_battery_amount, robot_4_battery_amount, robot_1_velocity, robot_2_velocity, robot_3_velocity, robot_4_velocity, battery_discharge, value=314)
            update_mission_length(mission_length, amount=314)
            update_input_amount(2, split_actions, robot_1_input_amount, robot_2_input_amount, robot_3_input_amount, robot_4_input_amount)

            

        elif split_actions[0] == "recharge_battery":
            recharge_battery(2, split_actions, battery_capacity)


        elif split_actions[0] == "recharge_input":
            recharge_input(2, split_actions, input_capacity)
            
            
        elif split_actions[0] == "harvest_crop":
            update_mission_length(mission_length, amount=20)
            update_battery_amount(2, split_actions, robot_1_battery_amount, robot_2_battery_amount, robot_3_battery_amount, robot_4_battery_amount, robot_1_velocity, robot_2_velocity, robot_3_velocity, robot_4_velocity, battery_discharge, value=100)
...
try:
    ...
    for obs_num in range(1, lines_num+1):
        ...
        # Time for recognition process
        time_sec = end - start

        # Stop if 1 hour recognition time is exceeded
        if (time_sec/60) > 60:
            break
