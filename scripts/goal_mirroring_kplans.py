...
# Select if Mean, median, min of K plans needs to be computed
approach = "median"
# wait for services
...
plan = ""
cost_plans = defaultdict(list)
goals_ideal_plans = dict()
goals_ideal_cost = dict()
...
# Add or remove the goal from knowledge base (kb)
...
# Execute the plan using the simulated actions
def execute_plan():
    # rospy.sleep(25.0)
...
# Check if plan is solved by the Metric-FF planner
...
# Update the numeric facts in kb since simulted actions does not do so.
...
# Finds median in list
def median(lst):
    n = len(lst)
    s = sorted(lst)
    return (s[n//2-1]/2.0+s[n//2]/2.0, s[n//2])[n % 2] if n else None

# Extracts K plans from the created .SOL files
def extract_k_plans(approach):
    # Loop through all the generated k plan files
    cost_arr = []
    all_plans = ''
    for i in range(1, 6):
        filename = "lpgplan_" + str(i) + ".SOL"
        ...
        # Checks if empty file and plan is complete when goals are achieved
        if not contains_digit:
            return 0.0, ""

        plan_lines = [l.strip() for l in lines if l.strip() and l[0].isdigit()]
        plan_string = '\n'.join(plan_lines)
        ...
        all_plans = all_plans + plan_string + '\n\n'


    if approach == "median":
        median_cost = median(cost_arr)
        return float(median_cost), all_plans
    
    elif approach == "mean":
        avg_cost = sum(cost_arr) / 5.0    
        return avg_cost, all_plans
    
    elif approach == "min":
        cost_arr.sort()
        return float(cost_arr[0]), all_plans


def plan_is_found(plans):


    cost, new_plan = extract_k_plans(approach=approach)
    if plan == new_plan:
        return False
    return True
...
try:
    ...
    with open(hyp, "r+") as f:
        ...
        for line in f:
            ...
            cost, plans = extract_k_plans(approach=approach)
            goals_ideal_plans[line] = plans
            goals_ideal_cost[line] = cost
    ...
    with open(log_file, "a") as log:
        with open(hyp, "r+") as hypothesis_goals:
            for line in hypothesis_goals:
                ...
                log.write("\t # Ideal Plan Cost: " + str(goals_ideal_cost[line]) + "\n \n")
    ...
    for obs_num in range(1, lines_num+1):
        ...
        with open(hyp, "r+") as candidate_goals:
            for line in candidate_goals:
                ...
                # Computes the average cost of the k plans and a string with the plans themselves.
                ideal = False
                cost, plans = extract_k_plans(approach=approach)
                suffix_plan_cost.append(plans)
                ...
                if found_plan:
                    goals_o_plans[line].append(plans)
                    cost_plans[line].append(cost)

                else:
                    goals_o_plans[line].append("Plan not found!")
                    cost_plans[line].append(0.0)
        ...
        for key, value in goals_o_plans.items():
            ...
            if "Plan not found!" in value[obs_num-1]:
                ...
                score = goals_ideal_cost[key] / (cost_obs + cost_plans[key][obs_num-1])
        ...
        with open(log_file, "a") as log:
            with open(hyp, "r+") as hypothesis_goals:
                for line in hypothesis_goals:
                    ...
                    log.write("\t # Plan complying with obs Cost: " + str(cost_plans[line][obs_num-1]) + "\n")
        ...
        # Stops the recognition process if 1 hour time limit is reached
        if (time_sec/60) > 60:
            break
